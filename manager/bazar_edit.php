...
<?php 
    if(isset($_POST['submit'])){
        ...
        $query="UPDATE bazardate set name ='$name',room_no='$room',phone='$phone' WHERE date='$date' ";
		...
		if($result)
		{
			...
			<strong>New Bazarkai Assigned Successfully!</strong>
			</div>"; 
			header('location:bazarschedule.php');
		}
		else
		{
		...
		<div class="addform">
			  ...
			  <h3>Today Date:&nbsp<?php echo date('d/m/y');?></h3><br>
			   ...
			   <form class="formmain" method="post" enctype="multipart/form-data">
					    ...
					    <label>Assigned Date  :&nbsp <span style="color:red">*</span> </label> <?php echo $_GET['date'] ;?><br><br>
						
						<label>Enter Name:&nbsp</label>
						<input  name="name" required="" placeholder="Bazarkari Name"><br><br>
						
						<label>Enter Room No:&nbsp</label>
						<input   name="room" required="" placeholder="Bazarkari Room No."><br><br>
						
						<label>Enter Phone No:&nbsp</label>
						<input  name="phone" required="" placeholder="Bazarkari Phone No."><br>
